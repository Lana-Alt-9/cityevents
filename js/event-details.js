...
function toggleWishlist() {
    ...
    if (wishlistBtn.classList.contains('active')) {
        ...
        wishlistBtn.innerHTML = '<i class="fas fa-heart me-2"></i>Added to Wishlist';
...
function validateTicketForm(data) {
    ...
    if (!data.type) errors.push('Please select a ticket type');
    if (!data.quantity) errors.push('Please select quantity');
...
function showTicketConfirmation(ticketData) {
    ...
    const confirmationHTML = `
        <div class="ticket-confirmation">
            ...
            <h3>Tickets Purchased Successfully!</h3>
            <div class="confirmation-details">
                <p><strong>Event:</strong> ${eventTitle}</p>
                <p><strong>Ticket Type:</strong> ${ticketTypeText}</p>
...
function initSimilarEvents() {
    ...
    similarEvents.forEach(event => {
        event.addEventListener('click', function() {
            ...
            showNotification(`Redirecting to ${eventTitle}...`, 'info');
